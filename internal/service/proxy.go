...
func (s *ProxyService) RegenerateProxyConfig(ctx context.Context) error {
	...
	config := s.generateVelocityConfig(servers)
...
func (s *ProxyService) generateVelocityConfig(servers []*models.MinecraftServer) string {
...
%s = "%s:25565"`, server.Name, server.Name))
	...
	// Set first server as default if any exist
	defaultServer := ""
	if len(tryList) > 0 {
		defaultServer = strings.Join(tryList, ", ")
	}
...
[query]
enabled = false
`, strings.Join(serverEntries, ""), defaultServer)
